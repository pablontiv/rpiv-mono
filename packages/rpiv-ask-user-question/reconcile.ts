 ...
 * in non-interactive runs unless Jev auto-answer is enabled, and present in
 * interactive ones. Mirrors the
...
/**
 * Strip-or-restore `ask_user_question` to match UI availability or the
 * user-owned Jev auto-answer opt-in. Reads the active tool list itself.
 * Idempotent: when the tool is already in the right state it leaves the active
 * set (and sibling tools) untouched.
 */
export function reconcileAskUserQuestionTool(
	pi: ExtensionAPI,
	...
	const shouldExpose = ctx.hasUI || autoAnswerState.enabled;
...
export function registerAskUserQuestionReconciler(
	...
	pi.on("before_agent_start", (_event, ctx) => reconcileAskUserQuestionTool(pi, ctx, autoAnswerState));
}
